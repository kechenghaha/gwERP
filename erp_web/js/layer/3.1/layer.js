  ...
  callback([
    ...
    '<div class="'+ doms[0] + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fixed ? '' : ';position:absolute;') +'">'
      ...
      + '<span class="layui-layer-setwin">'+ function(){
        var closebtn = ismax && config.title ? '<a class="layui-layer-min" href="javascript:;"><cite></cite></a>' : ''; // 必须有标题的清空下才能最小化
...
Class.pt.creat = function(){
  ...
  switch(config.type){
    case 0:
      ...
      config.btn = ('btn' in config) ? config.btn : layer.i18n.btnOk;
...
//      	+ config.content[0] + '"></iframe>';
  ...
  //config.type == 2 && layer.ie == 6 && that.layero.find('iframe').attr('src', content[0]); // 不需要ie6
...
Class.pt.offset = function(){
  ...
  that.offsetTop = that.offsetTop>0?that.offsetTop:0; // Top值不小于0
  ...
  layero.find('.'+ doms[6]).children('a').on('click', function(){
    ...
    if(index === 0){
      if(config.yes){
        ...
        close === false || layer.close(that.index); //添加返回值如果是false则关闭窗口
      } else if(config['btn1']){
